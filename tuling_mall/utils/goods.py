...
def get_categories():
    ...
    for channel in channels:
        ...
        if group_id not in categories:
            # 我们就把 组id 添加到 有序字典中, 并且作为 key值
            # value值 是 {'channels': [], 'sub_cats': []}
            categories[group_id] = {
                 'channels': [],
                 'sub_cats': []
            }
        ...
        cat2s = GoodsCategory.objects.filter(parent=cat1)  # 获取所有【二级分类实例】
        # cat1.goodscategory_set.all()
        for cat2 in cat2s:
            # 创建一个新的列表:
            cat2.sub_cats = []
            # 根据 cat2 的外键反向, 获取下一级(三级菜单)的所有分类数据, 并遍历:
            cat3s = GoodsCategory.objects.filter(parent=cat2) # 获取所有的【三级分类实例】
            for cat3 in cat3s:
